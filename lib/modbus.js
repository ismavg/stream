...
exports.tcprtu = {
	connect : function () {
		...
		socket.attach(exports.transports.serial.prepare(options), next);
	},
	server : function () {
		...
		var stream = exports.drivers.tcp.server(options);

		if (options.mode === "ascii") {
			return stream.attach(exports.transports.ascii.prepare(serial_options), next);
		} else {
			return stream.attach(exports.transports.serial.prepare(serial_options), next);
		}
	}
};
